...
const ensureGrantName = (value?: string): string => {
...
const ensureSanctionedAmount = (value: unknown): number => {
  const amount = parseNumber(value, Number.NaN);
  if (!Number.isFinite(amount) || amount <= 0) {
    throw new Error("Total sanctioned amount must be greater than zero");
  }
  return amount;
};

const buildGrantRecord = (draft: GrantRecordDraft): GrantRecord => {
  const id = draft.id ?? randomUUID();
  const name = ensureGrantName(draft.name);
  const totalSanctionedAmount = ensureSanctionedAmount(draft.totalSanctionedAmount);
  const currency = draft.currency && draft.currency.trim().length
  ...
  const payload: GrantRecordPayload = {
    id,
    name,
    fundingAgency: draft.fundingAgency?.trim() || undefined,
    program: draft.program?.trim() || undefined,
    sanctionNumber: draft.sanctionNumber?.trim() || undefined,
    sanctionDate: ensureIsoString(draft.sanctionDate),
    totalSanctionedAmount,
    currency,
    managingDepartment: draft.managingDepartment?.trim() || undefined,
    purpose: draft.purpose?.trim() || undefined,
    startDate: ensureIsoString(draft.startDate),
    endDate: ensureIsoString(draft.endDate),
    disbursements: [],
    expenditures: [],
    compliance: [],
  ...
  return normaliseGrantRecord(payload);
};

const applyGrantRecordUpdate = (record: GrantRecord, draft: GrantRecordDraft): boolean => {
  let changed = false;

  if (draft.name !== undefined) {
    const nextName = ensureGrantName(draft.name);
  ...
  if (draft.fundingAgency !== undefined) {
    const next = draft.fundingAgency?.trim() || undefined;
    if (next !== record.fundingAgency) {
      record.fundingAgency = next;
      changed = true;
    }
  }

  if (draft.program !== undefined) {
    const next = draft.program?.trim() || undefined;
    if (next !== record.program) {
      record.program = next;
      changed = true;
    }
  }

  if (draft.sanctionNumber !== undefined) {
    const next = draft.sanctionNumber?.trim() || undefined;
    if (next !== record.sanctionNumber) {
      record.sanctionNumber = next;
      changed = true;
    }
  }

  if (draft.sanctionDate !== undefined) {
    const next = ensureIsoString(draft.sanctionDate);
    if (next !== record.sanctionDate) {
      record.sanctionDate = next;
      changed = true;
    }
  }

  if (draft.totalSanctionedAmount !== undefined) {
    const amount = ensureSanctionedAmount(draft.totalSanctionedAmount);
    if (amount !== record.totalSanctionedAmount) {
      record.totalSanctionedAmount = amount;
      changed = true;
    }
  }

  if (draft.currency !== undefined) {
    const next = draft.currency && draft.currency.trim().length
      ? draft.currency.trim().toUpperCase()
      : "INR";
    if (next !== record.currency) {
      record.currency = next;
      changed = true;
    }
  }

  if (draft.managingDepartment !== undefined) {
    const next = draft.managingDepartment?.trim() || undefined;
    if (next !== record.managingDepartment) {
      record.managingDepartment = next;
      changed = true;
    }
  }

  if (draft.purpose !== undefined) {
    const next = draft.purpose?.trim() || undefined;
    if (next !== record.purpose) {
      record.purpose = next;
      changed = true;
    }
  }
  ...
  if (draft.startDate !== undefined) {
    const next = ensureIsoString(draft.startDate);
    if (next !== record.startDate) {
      record.startDate = next;
      changed = true;
    }
  }

  if (draft.endDate !== undefined) {
    const next = ensureIsoString(draft.endDate);
    if (next !== record.endDate) {
      record.endDate = next;
      changed = true;
    }
  }
  ...
  if (draft.metadata !== undefined) {
    record.metadata = draft.metadata ?? undefined;
    changed = true;
  }
...
export const createGrantRecord = async (
  startupId: string,
  draft: GrantRecordDraft,
): Promise<GrantCatalog> => {
  const catalogRecord = await getGrantCatalog(startupId);
  const grant = buildGrantRecord(draft);
  const now = new Date().toISOString();

  catalogRecord.catalog.grants.push(grant);
  catalogRecord.catalog.updatedAt = now;

  await saveGrantCatalogRecord(startupId, catalogRecord.catalog);
  return catalogRecord.catalog;
};

export const updateGrantRecord = async (
  startupId: string,
  grantId: string,
  draft: GrantRecordDraft,
): Promise<GrantCatalog> => {
  const catalogRecord = await getGrantCatalog(startupId);
  const grant = findGrantOrThrow(catalogRecord.catalog, grantId);
  const changed = applyGrantRecordUpdate(grant, draft);

  if (changed) {
    catalogRecord.catalog.updatedAt = new Date().toISOString();
    await saveGrantCatalogRecord(startupId, catalogRecord.catalog);
  }
...
export const deleteGrantRecord = async (
  startupId: string,
  grantId: string,
): Promise<GrantCatalog> => {
  const catalogRecord = await getGrantCatalog(startupId);
  const before = catalogRecord.catalog.grants.length;
  catalogRecord.catalog.grants = catalogRecord.catalog.grants.filter((grant) => grant.id !== grantId);

  if (catalogRecord.catalog.grants.length === before) {
